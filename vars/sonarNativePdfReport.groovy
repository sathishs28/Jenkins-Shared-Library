...
def call(Map config = [:]) {
    ...
    // ─── Helper: Colored Cell ───
    def coloredCell = { text, bgColor, textFont, align = Element.ALIGN_LEFT ->
        def cell = new PdfPCell(new Phrase(text, textFont))
        cell.backgroundColor = bgColor
        cell.horizontalAlignment = align
        cell.verticalAlignment = Element.ALIGN_MIDDLE
        cell.paddingTop = 8; cell.paddingBottom = 8
        cell.paddingLeft = 10; cell.paddingRight = 10
    ...
    alertTable.addCell(coloredCell(alertMsg, alertColor, fontAlert))
    doc.add(alertTable)
    doc.add(Chunk.NEWLINE)
    ...
    if (totalNewIssues > 0) {
        ...
        def warnCell = coloredCell("New Issues Detected: ${totalNewIssues} new issue(s) found in this commit. Please review before merging.", 
                                   new Color(255, 243, 205), new Font(Font.HELVETICA, 10, Font.BOLD, new Color(133, 100, 4)))
    ...
    def addSection = { title ->
        ...
        p.spacingAfter = 8
        doc.add(p)
        doc.add(new LineSeparator(1, 100, new Color(233, 236, 239), Element.ALIGN_CENTER, -2))
    }
    ...
    // ─── FOOTER ───
    doc.add(Chunk.NEWLINE)
    def footerLine = new LineSeparator(0.5f, 100, new Color(233, 236, 239), Element.ALIGN_CENTER, -2)
    doc.add(footerLine)
